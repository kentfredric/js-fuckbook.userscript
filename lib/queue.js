...
Queue = function(name) {
  var qname = name;
    ...
    Queue.log('running: ' + items[ pos ].name );
		items[ pos ].fn();
	};
	stash.next = function() {
		pos = pos + 1;
	...
	stash.add_repeater_maxed = function( name, test, success ) {
    var max_repeats = 100;
		var cb_code = function() {
      Queue.log('test repeater: ' + name );
      if ( max_repeats < 0 ) {
        Queue.log('test repeater exiting: ' + name + "( max repeats exceeded )");
        return;
      }
      max_repeats--;
			if ( test() ){
  ...
  stash.add_wait_maxed = function( name, test ){ 
		stash.add_repeater_maxed( name, test, function(){ 
			stash.dequeue();
		});
	};
